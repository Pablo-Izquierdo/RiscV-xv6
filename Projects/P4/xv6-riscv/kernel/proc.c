...
//static struct proc* allocthread(uint64* pagetable);
...
// Return 0 on success, -1 on failure.
int
growdifferentproc(int n, struct proc* differentProcess)
{
  uint sz;
  struct proc *p = differentProcess;
...
int
findgrowthread(int n, struct proc* p, uint64 addr)
{
  struct proc* t;
  for(t = proc; t < &proc[NPROC]; t++){
    if(t->asid == p->pid && t != p){  // the process with the same asid
      if(growthread(n, p->pagetable, t->pagetable, addr) == -1){
        printf("growthread failure\n");
        return -1;
      }
      t->sz = p->sz;
    }
...
int clone(void *fcn(void*), void *arg, void *stack){
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  uint64 stack_args[2]; 
  stack_args[1] =  (uint64)arg; //PC retorno
  stack_args[0] =  (uint64)0xffffffff; //cast uint64
    ...
    for(np = proc; np < &proc[NPROC]; np++){
      if(np->parent == p && np->soythread){
        ...
        if(np->state == ZOMBIE){
          ...
          if (copyout(np->pagetable, stack, (char*) &np->bottom_ustack, sizeof(uint64)) < 0) { //NO PASA VALOR
            printf("ERROR en copyout\n");
            release(&np->lock);
            release(&wait_lock);
            return -1;
          }
