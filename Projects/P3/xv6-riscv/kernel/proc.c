...
userinit(void)
{
  ...
  p->trapframe->epc = 0;      // user program counter //.MODNULL
...
fork(void)
{
  ...
  /* uvmcopy didn´t copy shmem to child, it only copy form addr 0 to the process sz pointer
  * due to we have shmem at the top (under trapframe) we have to give access (map) shmem to the child.
  */
  ...
  *(np->trapframe) = *(p->trapframe);
  ...
  np->state = RUNNABLE;
  for(int i=0; i<SHMEM; i++){ //.ADD
    if(p->shmem[i].VADDR != NULL){ //Give access shmem to child
      release(&np->lock);
      get_shmem_access(i,np->pid);
      acquire(&np->lock);
    }
  }
  release(&np->lock);
...
exit(int status)
{
  ...
  for(int i=0; i<SHMEM; i++){ //.ADD
    if (p->shmem[i].VADDR != NULL){
        pte = walk(p->pagetable, (uint64)p->shmem[i].VADDR, 0);
        *pte &= ~PTE_V; //we invalidate the page
...
/*
 * Reduce global counter for a shmem page
 * user don´t have acces to this function
*/
void reduce_counter_shmem_page (int page_number){ //.ADD
  shmem[page_number].numberofusers--;
}
